...
namespace EchoBot
{
    ...
    public class BotHost : IBotHost
    {
        ...
        public async Task StartAsync()
        {
            ...
            // EventLog provider on stripped Windows VMs throws RPC-unavailable
            // from inside async-void media callbacks (OnAudioMediaReceived,
            // VideoFramePlayer.DeliverFrameAsync). The exception aggregates into
            // "Unhandled exception. System.AggregateException: An error occurred
            // while writing to logger(s)" which is observed continuously in
            // lisa-bot.err.log. Filter + descriptor-removal proved unreliable
            // because WebApplicationBuilder re-adds providers via factories that
            // bypass our descriptor scan. Bulletproof fix: ClearProviders, then
            // re-add ONLY safe providers.
